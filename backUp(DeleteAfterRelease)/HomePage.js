const {
  ...
  USER_MENU_ARROW,
  MY_SETTINGS_LINK,
  SELECT_RACEHORSE_BUTTON,
} = require("./ZedRun");
const { EVENT_TAB } = require("./Racing");
const { WALLET_ICON } = require("./Wallet");
...
class HomePage {
  ...
  async clickOnAcceptButton() {
    ...
    try {
      await this.page.waitForSelector(ACCEPT_BUTTON, { timeout: 0 });
  ...
  async clickOnWalletIcon() {
    ...
    try {
      await this.page.waitForSelector(WALLET_ICON, { timeout: 0 });
  ...
  async checkIfAvatarPresent() {
    ...
    try {
      ...
      if (await this.page.isVisible(USER_AVATAR)) {
        return true;
      } else return false;
    } catch {
  ...
  async selectEventsTab() {
    console.log(
      "--- Zed Run Automation Framework: Select the Events tab on Racing page ---"
    );
    try {
  ...
  async validateStreamingWidgetSection() {
    console.log(
      "---- Zed Run Automation Framework: Validate the Streaming Widget Section on Home Page ---"
    );
    try {
  ...
  async validateDiscordWidgetSection() {
    console.log(
      "---- Zed Run Automation Framework: Validate the Discord Widget Section on Home Page ---"
    );
    try {
  ...
  async validateRaceSection() {
    console.log(
      "---- Zed Run Automation Framework: Validate the Race Section on Home Page ---"
    );
    try {
  ...
  async validateZEDSection() {
    console.log(
      "---- Zed Run Automation Framework: Validate the ZED Section on Home Page ---"
    );
    try {
  ...
  async validateFooterSection() {
    console.log(
      "---- Zed Run Automation Framework: Validate the Footer Section on Home Page ---"
    );
    try {
    } catch {
      throw new Error("");
  ...
  async clickOnUserMenuArrow() {
    console.log(
      "---- Zed Run Automation Framework: Click on user menu arrow ---"
    );
    try {
      await this.page.waitForSelector(USER_MENU_ARROW, { timeout: 0 });
      await this.page.click(USER_MENU_ARROW);
      await this.page.waitForLoadState();
    } catch {
      throw new Error("User menu arrow is not present!");
    }
  }

  async clickOnMySettingsLink() {
    console.log(
      "---- Zed Run Automation Framework: Click on My Settings link ---"
    );
    try {
      await this.page.waitForSelector(MY_SETTINGS_LINK, { timeout: 0 });
      await this.page.click(MY_SETTINGS_LINK);
      await this.page.waitForLoadState();
    } catch {
      throw new Error("My settings link is not present!");
    }
  }

  async clickOnSelectRaceHorseButton(){
    console.log(
        "---- Zed Run Automation Framework: Click on Transfer Horse button ---"
      );
      try {
        await this.page.waitForSelector(SELECT_RACEHORSE_BUTTON, { timeout: 0 });
        await this.page.click(SELECT_RACEHORSE_BUTTON);
        await this.page.waitForLoadState();
      } catch {
        throw new Error("Transfer horse button is not present!");
      }
  }

  async clickOnUserAvatar() {
    console.log(
      "---- Zed Run Automation Framework: Click on user avatar ---"
    );
    try {
      await this.page.waitForSelector(USER_AVATAR, {
        timeout: 0,
