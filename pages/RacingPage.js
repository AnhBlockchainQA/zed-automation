...
class RacingPage {
    ...
    async selectFirstEntryHasFeeEvent(){
        console.log("--- Zed Run Automation Framework: Wait until entry has a fee event shown then click on---");
    ...
    async getGateOpening(){
        ...
        let numberOfGate = listOfGate.map(number => {
    ...
    async selectHorserace(index) {
        await this.page.click(`//div[contains(@class,'pick-gate')]//div[@class='gate-group']/div[@class='gate-btn' and descendant::text()=${index}]`);
