...
class Metamask {
    ...
    async authenticate(browserContext: BrowserContext) {
        ...
        await this.pages[1].type(this.objects.TEXT_AREA_METAMASK_PASSPHRASE, data.seed_phrase);
        await this.pages[1].type(this.objects.TXT_METAMASK_PASSWORD, data.password);
        await this.pages[1].type(this.objects.TXT_METAMASK_PASSWORD_CONFIRM, data.password);
