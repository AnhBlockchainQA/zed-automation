...
describe('Home', () => {
  ...
  it('ZED-22 - Home is showing the Learn Dropdown List on the top navigation bar with the corresponding sub-links and redirects respectively', async () => {
    ...
    expect(await gettingStarted.getAttribute('href')).toContain(
      'https://guide.zed.run',
    );
    ...
    expect(await productPortal.getAttribute('href')).toContain(
      'https://product.zed.run/tabs/4-under-development',
    );
    ...
    expect(windows3[1].url()).toContain(
      'https://product.zed.run/tabs/15-under-consideration/tabs/4-under-development',
    );
  ...
  it('ZED-24 - Home is showing/loading the Streaming Video', async () => {
    ...
    if (await error.isVisible()) {
      await page.waitForLoadState();
      const errorIcon = await error.waitForSelector(
  ...
  it('ZED-25 - Home is showing the UP and COMING section with at least 3 Races in Card Containers', async () => {
    ...
    const length = await page.$$eval(
      home.objects.divCardsUpAndUpcoming,
      (items) => items.length,
    );
  ...
  it('ZED-27 - Home is showing the On Sale Section with a list of Horses shown in Card Containers with their respective price per animal', async () => {
    ...
    if (await soldOutShadow.isVisible()) {
      ...
      expect(
        await soldOutContent.$$eval('p', (nodes) =>
      ...
      ).toEqual(['https://discord.gg/zedrun']);
    } else {
  ...
  it('ZED-29 - Home is showing the Create your free stable section with START button on the right side of the content', async () => {
    ...
    const lblCreateFreeStableLegend =
      await home.lblPUpAndUpcomingCreateFreeStableLegend();
  ...
  it('ZED-30 - Home is showing In Stud section with a list of horses', async () => {
    ...
    if (await inStudHorses.isVisible()) {
      ...
      const OnSaleCards = await page.$$eval(
        home.objects.divInStudHorsesSection,
        (items) => items.length,
      );
      expect(OnSaleCards >= 1).toBe(true);
    } else {
      // implement code here.
      expect(false).toBeFalsy();
  ...
  it('ZED-97 - Home is showing up the modal with a warning message when the Metamask Extension has not been installed @fast', async () => {
    ...
    expect(await modalTitle.textContent()).toContain(
      `HAVE YOU INSTALLED METAMASK?`,
    );
    const modalLegend = await home.lblPLegendModalNonExtension();
    expect(await modalLegend.textContent()).toContain(
      `ZED has detected that your browser does not have MetaMask installed. MetaMask is widely used by many blockchain applications in order to keep your information secure.`,
    );
    const modalQuestion = await home.lblPLegendQuestionModalNonExtension();
    expect(await modalQuestion.innerText()).toContain(
      `What is MetaMask? Head over to our  FAQ`,
    );
